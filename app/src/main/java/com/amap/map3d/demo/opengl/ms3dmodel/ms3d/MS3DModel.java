...
import com.amap.map3d.demo.opengl.ms3dmodel.program.ShaderProgram;
...
public class MS3DModel{
	...
	protected MS3DModel(Context context){
		initShader(context);
	}
    //初始化着色器
	...
	public void draw(boolean isUpdate){//绘制模型
        ...
        for(int i=0; i<group_size; i++){
        	...
        	if(group.getMaterialIndex() > -1){
				...
				mProgram.setTextureAttribute(this.texCoordingBuffer[i], 2);
        	}
        	if(isUpdate){//更新顶点缓冲
	...
	//加载模型的方法
	public final static MS3DModel load(InputStream is, TextureManager manager,Context context){
		MS3DModel model = null;
		SmallEndianInputStream fis = null;
		try{
			//将输入流封装为SmallEndian格式的输入流
			fis = new SmallEndianInputStream(is);
			model = new MS3DModel(context);
